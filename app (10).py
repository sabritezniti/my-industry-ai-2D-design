...
# import cv2  # Remplacé par Pillow pour compatibilité
import io
import os
import tempfile
...
class ParametricParser:
    ...
    def parse_single_operation(self, op_text):
        ...
        # ── Substituer TOUTES les variables avant tout parsing ──
        resolved_text = op_text
        for vn, vv in self.variables.items():
            resolved_text = resolved_text.replace(f"${vn}", str(vv))
            resolved_text = re.sub(r'\b' + re.escape(vn) + r'\b', str(vv),
                                   resolved_text, flags=re.IGNORECASE)

        op_lower = resolved_text.lower().strip()
        if not op_lower:
            return []
        ...
        if any(word in op_lower for word in ["rectangle", "rect", "carré", "carre", "square"]):
            nums = self.extract_numbers(resolved_text)
            if len(nums) >= 2:
                # Chercher le format WxH en priorité
                dim_match = re.search(r"(\d+\.?\d*)\s*[xX]\s*(\d+\.?\d*)", resolved_text)
                if dim_match:
                ...
                x, y = 0, 0
                pos_match = re.search(r"à\s*\((-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)\)", resolved_text)
                if pos_match:
        ...
        # CERCLE
            ...
            if radius is None and nums:
                radius = nums[0]
                if "diam" in op_lower or "ø" in resolved_text:
                    radius = radius / 2

            if radius is not None:
                positions = self.calculate_position(resolved_text)
        ...
        # LIGNE
            ...
            coords = re.findall(r"\((-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)\)", resolved_text)
            if len(coords) >= 2:
        ...
        # POLYGONE
            ...
            nums = self.extract_numbers(resolved_text)
            if len(nums) >= 2: sides = int(nums[0])
        ...
        # TEXTE
            ...
            text_match = re.search(r'["\'](.+?)["\']', resolved_text)
...
def image_to_dxf_parametric(image_file, threshold=128, simplify=True, 
    ...
    """Convertit une image en DXF avec paramètres de modification - Version Pillow (sans OpenCV)"""
    ...
    # Vectorisation avec contour tracing simple (sans OpenCV)
    doc = ezdxf.new("R2010")
    doc.header["$INSUNITS"] = 4
    msp = doc.modelspace()
    ...
    # Méthode simple: détecter les bords avec différences de pixels
    # et créer des rectangles pour les zones noires
    h, w = binary.shape
    visited = np.zeros((h, w), dtype=bool)

    def trace_contour(start_x, start_y):
        """Trace un contour simple à partir d'un point de départ"""
        contour = []
        ...
        while stack:
            x, y = stack.pop()
            if x < 0 or x >= w or y < 0 or y >= h or visited[y, x] or binary[y, x] == 255:
                continue
            ...
            # Voisins (4-connectivité)
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                stack.append((x + dx, y + dy))

        if len(contour) < 10:  # Ignorer les très petites zones
            return None

        # Créer un rectangle englobant comme approximation
        return [(min_x, min_y), (max_x, min_y), (max_x, max_y), (min_x, max_y), (min_x, min_y)]

    # Parcourir l'image et trouver les contours
    for y in range(h):
        for x in range(w):
            if binary[y, x] == 0 and not visited[y, x]:
                contour_points = trace_contour(x, y)
                if contour_points:
                    all_contours_data.append(contour_points)
                    msp.add_lwpolyline(contour_points, close=True)
                    entities_count += 1
...
def repair_dxf(dxf_file):
    ...
    try:
        ...
        try:
            doc, _ = ezdxf.recover.readfile(dxf_file)
        except:
...
def dxf_to_svg_preview(doc, width=800, height=600):
    ...
    for entity in msp:
        try:
            ...
            if dxftype == "LWPOLYLINE":
                points = list(entity.get_points("xy"))   # ← list() obligatoire
                if len(points) > 1:
                    path_data = "M " + " L ".join(
                        f"{transform(p)[0]:.2f},{transform(p)[1]:.2f}" for p in points
                    )
                    if entity.closed:
                        path_data += " Z"
                ...
                svg_parts.append(
                    f'<circle cx="{center[0]:.2f}" cy="{center[1]:.2f}" r="{r:.2f}"/>'
                )
                ...
                # Calcul du span en tenant compte du sens trigo (CCW)
                ...
                p2 = transform((x2, y2))
                r  = radius * scale
                # DXF = CCW → SVG: sweep=0 (CCW en coords DXF = CW en pixels donc sweep=1)
                svg_parts.append(
                    f'<path d="M {p1[0]:.2f},{p1[1]:.2f} '
                    f'A {r:.2f},{r:.2f} 0 {large_arc} 0 {p2[0]:.2f},{p2[1]:.2f}"/>'
                )

            elif dxftype == "TEXT":
                insert = transform(entity.dxf.insert)
                text   = entity.dxf.text
                h      = max(entity.dxf.height * scale, 12)
                svg_parts.append(
                    f'<text x="{insert[0]:.2f}" y="{insert[1]:.2f}" '
                    f'font-size="{h:.1f}" fill="#1f77b4" stroke="none" '
    ...
    # Envelopper dans du HTML pour st.components.v1.html
    return (
        '<!DOCTYPE html><html><head>'
        '<meta charset="utf-8">'
        '<style>body{margin:0;padding:0;background:#f8f9fa;}'
...
with tab3:
    ...
    **Réparations automatiques:**
    ...
    with col1:
        ...
        if uploaded_dxf:
            ...
            if st.button("🔧 Réparer le DXF", type="primary", use_container_width=True):
                ...
                # Clear previous repair results
                for key in ['repaired_dxf_bytes', 'repaired_dxf_repairs', 'repaired_dxf_svg']:
                ...
                with st.spinner("Analyse et réparation..."):
                    try:
                        ...
                        if doc is None:
                            error_msg = repairs[0] if isinstance(repairs, list) else str(repairs)
    ...
    with col2:
        if 'repaired_dxf_repairs' in st.session_state and isinstance(st.session_state.repaired_dxf_repairs, list):
            ...
            for repair in st.session_state.repaired_dxf_repairs:
                st.write(f"✓ {repair}")
            st.markdown('</div>', unsafe_allow_html=True)
        elif 'repaired_dxf_repairs' in st.session_state:
            st.error(f"❌ {st.session_state.repaired_dxf_repairs}")
            ...
            if 'repaired_dxf_bytes' in st.session_state and st.session_state.repaired_dxf_bytes is not None:
                if isinstance(st.session_state.repaired_dxf_bytes, (bytes, bytearray)):
                    st.download_button(
                    ...
                    st.warning("⚠️ Données DXF invalides")
        else:
